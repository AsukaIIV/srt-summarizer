...
### 方式三：运行 Windows `.exe` 发行版

如果你使用的是打包后的 Windows 发行版：

- 直接双击 `SRT SUMMARIZER.exe`
- **不需要**额外安装 Python
- 发行版可内置 `ffmpeg.exe`
- 这时不再经过 `start.bat`，也不会在启动时执行 pip 安装

---

## 如何构建 Windows `.exe`

当前推荐使用 **PyInstaller onefile** 模式打包，这样最终可以只分发一个 `exe` 文件。

### 构建前准备

先把 Windows x64 的 `ffmpeg.exe` 放到：
...
2. 安装 `pyinstaller`
3. 校验内置 `ffmpeg.exe` 是否存在
4. 生成 `dist/SRT SUMMARIZER.exe` 单文件发行版
5. 把 Windows 文件版本信息写入最终 exe
...
```text
dist/SRT SUMMARIZER.exe
```

发行版运行时：

- 优先使用随程序打包的内置 `ffmpeg`
- 若未找到内置 `ffmpeg`，再尝试使用系统环境中的 `ffmpeg`
- 若仍未找到，则视频任务会按当前实现回退到 OpenCV 规则抽帧；如果最终没有有效截图，任务仍会直接失败
- 单文件 exe 启动时会把内置运行时文件临时解压到系统临时目录，因此启动速度通常会比 onedir 稍慢，这是正常现象
